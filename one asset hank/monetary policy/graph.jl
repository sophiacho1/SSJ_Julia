...
function DAG_get_ss(economy::DAG_Rep, stop::String, hh_calibration, beta_pol_type)
    ...
    for block in economy.topsorted
        ...
        if block.name == "HA"
            DAG_ss["A"], DAG_ss["C"], hh_ss_all_vals = solve_ss(block, hh_calibration, beta_pol_type)
        end
        ...
        for out in keys(block.outs)
            ...
            try
                for children in economy.ins_blocks[out]
                    economy.blocks[children].ins[out] = block.outs[out]
                end
