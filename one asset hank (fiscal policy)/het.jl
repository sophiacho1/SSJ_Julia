...
+="
function solve_ss(hh::HH_Block, ins)
  # Create block instance
  hh.all_ins = merge(hh.ins, ins)

  a_grid, z, e_grid, Pi = mk_grids_and_income(hh.all_ins["rho_e"], hh.all_ins["sd_e"], hh.all_ins["n_e"], 
                                      hh.all_ins["min_a"], hh.all_ins["max_a"], hh.all_ins["n_a"], hh.all_ins["Z"])

  ss = steady_state(Pi, a_grid, z, hh.all_ins["r"], hh.all_ins["beta"], hh.all_ins["eis"])
  
  # Define the function to be optimized
  function objective_function(beta)
    result = steady_state(ss["Pi"], ss["a_grid"], z, ss["r"], beta, ss["eis"])["A"] - hh.all_ins["B"]
    return result
  end
  
  # Find the root using find_zero with the Bisection method
  beta_ge = find_zero(objective_function, (0.75, 0.9), Bisection())
...
function get_shocked_inputs_no_compound(ss, shock, Y, B, B_prev, G, h)
  shocked_inputs = Dict()
  for key in keys(shock) # Has only one key
    if key == "r"
      shocked_inputs[key] = ss[key] + h * shock[key] # Change in interest rate
...
function step1_backward(ss, shock, Y, B, B_prev, G, T, h, compound)
  ...
  for s in 0:T-1
    if s == 0
      ...
      shoscked_inputs = []
      if compound
        shocked_inputs = get_shocked_inputs(ss, shock, Y, B, B_prev, G, h)
      else
        shocked_inputs = get_shocked_inputs_no_compound(ss, shock, Y, B, B_prev, G, h)
      end
...
function ha_jacobian(ss, shocks, Y, B, B_prev, G, T, compound)
  # Step 1: for all shocks i, allocate to curlyY[o][i] and curlyD[i]
  curlyY = Dict("A" => Dict(), "C" => Dict())
  curlyD = Dict()
  for (i, shock) in shocks
      curlyYi, curlyD[i] = step1_backward(ss, shock, Y, B, B_prev, G, T, 1E-4, compound)
...
function impulse_map(Ys, rs, Bs, Bs_prev, Gs, ss, T)

  Z = (Ys .- (1 .+ rs) .* Bs_prev + Bs - Gs)
  zs = Z * ss["e_grid"]'

  z_dict = zeros(end_T, 10)
  for i in 1:T
      z_dict[i, :] = (zs[i, :] .- ss["z"]) ./ ss["z"]
  end

  impulse = household_impulse(ss, Dict("r" => rs .- ss["r"], "z" => z_dict), T)
  
  return impulse["A"] - Bs, Ys - impulse["C"] - Gs, impulse
end